...
namespace Draw3D
{
    public partial class FormGraph : Form
    {
        ...
        private void поискВГлубСОгремToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Program.formMyCustomDialog.Text = "Поиск в глубину (стек)";
            if (Program.formMyCustomDialog.ShowDialog() == DialogResult.OK)
            {
                int n = Lib.graph.DepthSearchStack1(Lib.NumNode0, Graph.Nodes[Lib.NumNode].name, Lib.strictNodes);
                Text = "Path: " + Lib.myStack.StackToStr();
                MyDraw(false);
            }
        }
